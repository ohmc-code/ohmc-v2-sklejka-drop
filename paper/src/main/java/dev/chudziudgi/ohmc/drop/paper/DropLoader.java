...
public class DropLoader implements PluginLoader {

    private static final Logger LOGGER = Logger.getLogger(DropLoader.class.getName());
    ...
    public void classloader(@NotNull PluginClasspathBuilder classpathBuilder) {
        try {
            ...
            PluginLibraries pluginLibraries = this.loadLibraries();
    ...
    private PluginLibraries loadLibraries() {
        try (InputStream inputStream = this.getClass().getResourceAsStream(LIBRARIES_JSON_PATH)) {
            if (inputStream == null) {
    ...
    private record PluginLibraries(@NotNull Map<String, String> repositories, @NotNull List<String> dependencies) {
        ...
        public Stream<RemoteRepository> asRepositories() {
            return this.repositories.entrySet()
                    .stream()
